...
func _ready():
	var anim:Animation=load('res://slidingAnim.res')
	...
	for i in track_count:
		...
		skip_by=0
		for j in key_count:
			var rounded=floor(anim.track_get_key_time(i,j-skip_by)*6)
			if abs(anim.track_get_key_time(i,j-skip_by)*6-rounded)>0.05:
